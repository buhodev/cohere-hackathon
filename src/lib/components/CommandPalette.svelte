<script lang="ts">
	...
	import { getOS, switchLocale, replaceLocaleInUrl } from '$lib/utils';
	...
	const QuickActions: SearchItems = [
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.TOGGLE_DARK_MODE(),
			id: uid(),
		...
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.CHANGE_LANGUAGE(),
			id: uid(),
			callback: () => {
		...
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.CONFETTI(),
			id: uid(),
		...
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.GO_TO_CHAT(),
			id: uid(),
			callback: () => {
		...
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.GO_HOME(),
			id: uid(),
			callback: () => {
		...
		{
			name: $LL.COMMAND_PALETTE.QUICK_ACTIONS.GO_TO_PLAYGROUND(),
			id: uid(),
			callback: () => {
		...
		>
			<div class="relative">
				...
				<input
					...
					placeholder={$LL.COMMAND_PALETTE.INPUT_PLACEHOLDER()}
				/>
			</div>
									...
									>
										...
										{:else}
											<span
												...
												>{$LL.COMMAND_PALETTE.JUMP_TO()}</span
											>
										{/if}
									</li>
								{/each}
						...
						>
							<svg
								class="h-6 w-6 flex-none text-neutral-900
									{active ? 'dark:text-white' : 'text-opacity-40 dark:text-neutral-500'}"
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
								stroke-width="1.5"
			...
			{:else}
				...
				<div class="py-14 px-6 text-center sm:px-14">
					...
					<p class="mt-4 text-sm text-neutral-900 dark:text-neutral-200">
						{$LL.COMMAND_PALETTE.EMPTY_STATE()}
					</p>
				</div>
			{/if}
